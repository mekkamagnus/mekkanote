...
- **Frontend**: HTMX for dynamic interactions, Hono framework
- **Styling**: Tailwind CSS + DaisyUI
- **Backend**: Hono web framework with server-side HTML rendering
...
│   └── server.ts          # Hono server with HTMX endpoints
...
End-to-end tests use Playwright to verify the HTMX application works correctly from the user's perspective:
...
**E2E Test Coverage:**
- Notes management (create, edit, delete, org-mode rendering)
- Search functionality (real-time search, empty states)
- Navigation (drawer, folders, settings, HTMX interactions)
- HTMX-specific testing (attributes, requests, partial updates)

**Linux ARM64 Note:** E2E tests use Chromium which has good ARM64 support. Firefox and WebKit may have limited support on ARM64 systems.
...
4. Run `deno task test` to ensure all unit tests pass
5. Run `deno task test:e2e` to ensure all E2E tests pass
